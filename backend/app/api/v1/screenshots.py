...
def _employee_timezone_rows(
    ...
    return [(employee_id, timezone_name or "UTC") for employee_id, timezone_name in db.execute(statement)]
...
def list_screenshots(
    ...
    if start_time:
        if selected_employee_rows:
            start = _local_day_bounds(start_time, selected_employee_rows[0][1])[0]
        else:
            start = day_bounds(start_time)[0]
        statement = statement.where(Screenshot.captured_at >= start)
    if end_time:
        if selected_employee_rows:
            end = _local_day_bounds(end_time, selected_employee_rows[0][1])[1]
        else:
            end = day_bounds(end_time)[1]
